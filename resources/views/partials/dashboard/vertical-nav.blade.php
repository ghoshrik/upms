<ul class="navbar-nav iq-main-menu"  id="sidebar">
    ...
    <li class="nav-item static-item">
        ...
        @else
        @php
            $side_menu = \App\Models\MenuPermission::join('menus', 'menu_permissions.menu_id', '=', 'menus.id')->where('parent_id', '=', '0')
                ->where('userType_id', '=', Auth::user()->user_type)
                ->get();
            $accessPermission = \App\Models\AccessMaster::join('access_types', 'access_masters.access_type_id', '=', 'access_types.id')
                ->join('menus', 'access_types.menu_id', '=', 'menus.id')->join('users', 'access_masters.user_id', '=', 'users.id')
        ...
        @endphp
            ...
            @endforeach
            @if ($accessPermission)
                @foreach ($accessPermission as $key=> $permission)
                <li class="nav-item">
                    <a class="nav-link" aria-current="page" href="#">
                        <i class="icon">
                            <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <li class="nav-item">
        <a class="nav-link" aria-current="page" href="#" target="_blank">
            ...
            <span>Design System<span class="badge rounded-pill bg-success">New</span></span>
        </a>
    </li> --}}
