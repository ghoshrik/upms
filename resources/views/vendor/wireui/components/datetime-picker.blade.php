<div x-data="wireui_datetime_picker({
    ...
    <x-dynamic-component :component="WireUiComponent::resolve('input')"
        ...
        :prepend="$prepend" readonly x-on:click="togglePicker" x-bind:value="model ? getDisplayValue() : null">
            ...
            <x-slot name="append">
                <div class="absolute inset-y-0 right-3 z-5 flex items-center justify-center">
                    <div
                        ...
                        <x-dynamic-component :component="WireUiComponent::resolve('icon')" class="cursor-pointer w-5 h-5" :name="$rightIcon"
    ...
    <div class="fixed inset-0 z-20 overflow-y-auto sm:absolute sm:inset-auto sm:top-10 sm:mt-1 sm:right-0" x-cloak
        style="display: none;" x-show="popover" x-on:click.outside="closePicker"
        ...
        <div class="flex items-end justify-center min-h-screen sm:h-96 sm:items-start"
            ...
            <div class="fixed inset-0 bg-secondary-400 bg-opacity-60 transition-opacity sm:hidden
                ...
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-on:click="closePicker"
                aria-hidden="true">
            </div>
            ...
            <div class="w-full rounded-t-md border border-secondary-200 bg-white shadow-lg
                ...
                <div x-show="tab === 'date'" class="space-y-5">
                    @unless ($withoutTips)
                        <div class="grid grid-cols-3 gap-x-2 text-center text-secondary-600">
                            <x-dynamic-component :component="WireUiComponent::resolve('button')"
                                class="bg-secondary-100 border-none dark:bg-secondary-800" x-on:click="selectYesterday"
                                label="{{ __('wireui::messages.datePicker.yesterday') }}" />

                            <x-dynamic-component :component="WireUiComponent::resolve('button')"
                                class="bg-secondary-100 border-none dark:bg-secondary-800" x-on:click="selectToday"
                                label="{{ __('wireui::messages.datePicker.today') }}" />

                            <x-dynamic-component :component="WireUiComponent::resolve('button')"
                                class="bg-secondary-100 border-none dark:bg-secondary-800" x-on:click="selectTomorrow"
                                label="{{ __('wireui::messages.datePicker.tomorrow') }}" />
                        </div>
                    @endunless

                    <div class="flex items-center justify-between">
                        <x-dynamic-component :component="WireUiComponent::resolve('button')" class="rounded-lg shrink-0" x-show="!monthsPicker"
                            x-on:click="previousMonth" icon="chevron-left" flat />

                        <div
                            class="w-full flex items-center justify-center gap-x-2 text-secondary-600 dark:text-secondary-500">
                            <button class="focus:outline-none focus:underline" x-text="monthNames[month]"
                                x-on:click="monthsPicker = !monthsPicker" type="button">
                            </button>
                            <input
                                class="w-10 sm:w-14 appearance-none p-0 ring-0 border-none focus:ring-0 focus:outline-none dark:bg-secondary-800"
                        ...
                        <div class="absolute inset-0 bg-white dark:bg-secondary-800 grid grid-cols-3 gap-3"
                            ...
                            <template x-for="(monthName, index) in monthNames" :key="`month.${monthName}`">
                                <x-dynamic-component :component="WireUiComponent::resolve('button')"
                                    ...
                                    x-on:click="selectMonth(index)" xs x-text="monthName" />
                            </template>
                        </div>
                                ...
                                <div class="flex justify-center picker-days">
                                    <button
                                        ...
                                        :class="{
                                            ...
                                            'text-secondary-400 dark:text-secondary-600': date.isDisabled || date
                                                .month !== month,
                                            'text-primary-600 border border-primary-600 dark:border-gray-400': date
                                                .isToday && !date.isSelected,
                                            'disabled:text-primary-400 disabled:border-primary-400': date.isToday && !
                                                date.isSelected,
                ...
                <div x-show="tab === 'time'" x-transition>
                    <x-dynamic-component :component="WireUiComponent::resolve('input')" id="search.{{ $attributes->wire('model')->value() }}"
                        label="Select time" x-model="searchTime" x-bind:placeholder="modelTime ? modelTime : '12:00'"
                        x-ref="searchTime" x-on:input.debounce.150ms="onSearchTime($event.target.value)" />
                        ...
                        <template x-for="time in filteredTimes">
                            <button
                                ...
                                <span
                                    ...
                                    <x-dynamic-component :component="WireUiComponent::resolve('icon')" name="check" class="h-5 w-5" />
                                </span>
                            </button>
                        </template>
                    </div>
