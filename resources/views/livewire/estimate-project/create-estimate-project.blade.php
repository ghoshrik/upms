                    ...
                    <div class="row">
                        <div class="row">
                            <div class="mb-2 col col-md-6 col-lg-6 col-sm-12 col-xs-12">
                                ...
                                {{-- <div wire:ignore>
                                    <div id="quillDescriptionEditor"></div>
                                </div> --}}
                        ...
                        @if (!empty($estimateData))
                            @if ($estimateData['item_name'] == 'SOR')
                                <div class="row" style="transition: all 2s ease-out"
                                    wire:key='{{ $estimateData['item_name'] }}' x-data="{ showSearch: false }">
                                    <div class="col">
                                        <div class="form-group">
                                    ...
                                    <div x-show="!showSearch" class="col">
                                        <div class="form-group">
                                            <x-select wire:key="table_no" label="Table No"
                                                placeholder="Select Table No" wire:model.defer="estimateData.table_no"
                                        ...
                                        <div class="form-group">
                                            @if ($estimateData['unit_id'] == 0 || $estimateData['unit_id'] == '')
                                                <x-select wire:key="unit_{{ rand(1, 1000) }}" label="Unit"
                                                    placeholder="Select Unit" wire:model.defer="estimateData.unit_id">
                                            ...
                                            @else
                                                <x-input wire:key='unit_{{ rand(1, 1000) }}' label="Unit Name"
                                                    placeholder="Unit Name" wire:model.defer="estimateData.unit_id"
                                                    readonly />
                                            @endif
                                        </div>
                                    </div>
    ...
    {{--    @if ($showOtherModal)
        <div class="modal fade show" style="display: block;" tabindex="-1" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Long Description</h5>
                        <button type="button" class="close" wire:click="$set('showOtherModal', false)">
                            <span>&times;</span>
                        </button>
                    </div>
                    <div class="modal-body" wire:ignore>
                        ...
                        <script>
                            ...
                            quill.on('text-change', function() {
                                @this.set('estimateData.other_name', quill.root.innerHTML);
                            });
                        </script>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary"
                            wire:click="$set('showOtherModal', false)">Close</button>
                        <button type="button" class="btn btn-primary" wire:click="saveDescription">Save
                            changes</button>
                    </div>
                </div>
            </div>
...
<script>
    /*  document.addEventListener('DOMContentLoaded', function() {
        ...
        var Descriptionquill = new Quill('#quillDescriptionEditor', {
            theme: 'snow',
            placeholder: 'Enter your text here...',
            modules: {
                toolbar: [
        ...
        Descriptionquill.on('text-change', function() {
            @this.set('sorMasterDesc', quill.root.innerHTML);
        });
