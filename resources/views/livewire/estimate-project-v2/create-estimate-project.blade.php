<div>
                    ...
                    <div class="row">
                        ...
                        @if (!empty($estimateData))
                            @if ($estimateData['item_name'] == 'SOR')
                                <div class="row" style="transition: all 2s ease-out" wire:key='{{ $estimateData['item_name'] }}' x-data="{ showSearch: false }">
                                    <div class="col">
                                        <div class="form-group">
                                            ...
                                            <x-select wire:key="estimate_no" label="{{ __('Select Estimate') }}"
                                                ...
                                                @isset($fatchDropdownData['estimatesList'])
                                                    @foreach ($fatchDropdownData['estimatesList'] as $estimate)
                                                        <x-select.option
                                                            label="{{ $estimate['estimate_id'] . ' - ' . $estimate['estimate_master_desc'] }}"
    ...
    @if ($addedEstimate != null || Session::has('addedProjectEstimateV2Data') || $editEstimate_id != '')
        <div x-transition.duration.500ms>
            <livewire:estimate-project-v2.added-estimate-project-list :addedEstimateData="$addedEstimate" :sorMasterDesc="$sorMasterDesc"
