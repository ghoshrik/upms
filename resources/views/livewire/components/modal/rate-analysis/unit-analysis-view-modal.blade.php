                            ...
                            <tbody class="metatable" id="metatable_id"
                                ...
                                @if (isset($rateAnalysisArray[$unit_id]) && isset($rateAnalysisArray[$unit_id]['metadata']))
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align:center;">{{ $part_no }}<?php echo $index + 1; ?></td>
                                        <td style="text-align:center;">
...
<script>
    ...
    $(document).ready(function() {
        ...
        var editRate_id = @json($editRate_id);
        var arrayCount = @json($arrayCount);
        var part_no = @json($part_no);
        ...
        $('#finalSubmitBtn').click(function() {
            ...
            window.Livewire.emit('submitGrandTotal', grandTotal, "{{ $unit_id }}");
            $("#myInput").modal('hide');
        });
        ...
        function checkSelectAll() {
            var $rows = $('#metatable_id tr');
            var $checkedRows = $rows.find('input.rowCheckbox:checked');

            if ($rows.length > 0 && $rows.length === $checkedRows.length) {
        ...
        // Check on row checkbox change
        $('#metatable_id').on('change', 'input.rowCheckbox', function() {
            checkSelectAll();
        });
        ...
        $('#selectAllCheckbox').on('change', function() {
            ...
            $('#metatable_id input.rowCheckbox').prop('checked', isChecked);
            $("#totalOnSelected").prop("disabled", !isChecked);
            checkSelectAll();
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(rateAnalysisArray1, function(index, metadata) {
                        ...
                        newRow.append('<td style="text-align:center;">' + part_no + (sno +
                                1) +
                            '</td>');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(rateAnalysisArray1, function(index, metadata) {
                        ...
                        newRow.append('<td style="text-align:center;">' + part_no +
                            (sno +
                                1) + '</td>');
                        newRow.append('<td style="text-align:center;">' +
                            metadata.type + (metadata.remarks ? ' (' +
    ...
    function checkRowCountAndConfirm() {
        setTimeout(function() {
            var rowCount = $('#dataTable1 tbody#metatable_id tr').length;
            if (rowCount === 1) {
                $("#finalSubmitBtn").prop("disabled", false);
                var userResponse = confirm(
                    'There is only one row in the table. Do you want to proceed?\n\nClick OK to calculate or Cancel to Add more rows.'
                );
                if (userResponse) {
                    $('#selectAllCheckbox').click();
                    $("#totalOnSelected").click();
                }
            }
