                                ...
                                <tbody>
                                    @forelse ($projects as $key => $project)
                                        <tr>
                                            ...
                                            <td class="text-wrap">{{ $project->site }}</td>
                                            <td>
                                                @php
                                                    $project_plans = $project->plans;
                                                    $project_estimates = $project->estimates;
                                                @endphp
                                                <button wire:click="openProjectPlans({{ $project->id }})" type="button"
                                            ...
                                            <td>
                                                <button wire:click="openEstimate({{ $project->id }})" type="button" class="btn btn-soft-primary btn-sm px-3 py-2.5 m-1 rounded">
                                                    @if (count($project_estimates) > 0)
                                                        <x-lucide-eye class="w-4 h-4 text-gray-500" /> View
                                                        <span
                                                            class="top-0 position-absolute start-100 translate-middle badge rounded-pill bg-primary">
                                                            {{ count($project_estimates) }}
                                                            <span class="visually-hidden">unread messages</span>
                                                        </span>
                                                    @else
                                                        <x-lucide-plus class="w-4 h-4 text-gray-500" /> Add
                                            ...
                                            {{-- <td class="text-center">
                                                <button
                                                    wire:click="fromEntryControl({ 'formType': 'edit', 'id': {{ $project->id }} })"
