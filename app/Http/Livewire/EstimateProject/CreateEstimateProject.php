...
class CreateEstimateProject extends Component
{
    ...
    public function addEstimate()
    {
        // dd($this->estimateData);
        $this->validate();
        $sessionValue = ($this->editEstimate_id != '') ? Session()->get('editProjectEstimationTotal' . $this->editEstimate_id) : Session()->get('projectEstimationTotal');
        if ($sessionValue == 1) {
            $this->notification()->error(
                $title = 'Total is already performed.'
            );
        } else {
            $this->part_no = strtoupper($this->part_no);
