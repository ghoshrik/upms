...
{
    use Actions;
    protected $listeners = ['getRowValue', 'closeModal', 'editEstimate', 'addEstimate'];
    ...
    public function getDeptEstimates()
    {
        ...
        $this->estimateData['description'] = '';
        $this->estimateData['total_amount'] = '';
        $this->fatchDropdownData['estimatesList'] = EstimateMasterV2::select('estimate_id', 'dept_id', 'estimate_master_desc')->where([['dept_id', $this->estimateData['dept_id']]])->get();
    }
    ...
    public function getEstimateDetails()
    {
        ...
        $this->fatchDropdownData['estimateDetails'] = EstimateMasterV2::where('estimate_id', $this->estimateData['estimate_no'])->first();
        ...
        $this->estimateData['total_amount'] = round($this->fatchDropdownData['estimateDetails']['total_amount']);
        $this->estimateData['description'] = $this->fatchDropdownData['estimateDetails']['estimate_master_desc'];
        $this->estimateData['qty'] = 1;
    ...
    public function addEstimate($data = '', $parentId = '')
    {
        ...
        if (Auth::user()->department_id == 26) {
            ...
            $this->addedEstimate['qty'] = ($this->estimateData['qty'] == '') ? 0 : (($parentId == '') ? $this->estimateData['qty'] : 1);
        }

        if (isset($this->estimateData['rate_det']) && $this->estimateData['rate_det'] != '') {
            $this->addedEstimate['rate'] = $this->estimateData['total_amount'];
        } else {
            $this->addedEstimate['rate'] = ($this->estimateData['rate'] == '') ? 0 : $this->estimateData['rate'];
        }
        if ($this->addedEstimate['unit_id'][0] === '%') {
            $this->addedEstimate['total_amount'] = round($this->estimateData['total_amount'] / 100);
        } else {
            $this->addedEstimate['total_amount'] = round($this->estimateData['total_amount']);
        }
        ...
        $this->addedEstimate['p_id'] = ($parentId == '') ? 0 : $parentId;
        $this->addedEstimate['rate_det'] = (isset($this->estimateData['rate_det'])) ? $this->estimateData['rate_det'] : '';
        $this->addedEstimateUpdateTrack = rand(1, 1000);
        $this->estimateData['item_number'] = '';
        $this->estimateData['estimate_no'] = '';
        $this->estimateData['rate_no'] = '';
        ...
        if ($parentId != '') {
            $this->resetExcept(['addedEstimate', 'showTableOne', 'addedEstimateUpdateTrack', 'sorMasterDesc', 'fatchDropdownData', 'part_no', 'editEstimate_id']);
        } else {
    ...
    {
        return view('livewire.estimate-project-v2.create-estimate-project');
    }
}
