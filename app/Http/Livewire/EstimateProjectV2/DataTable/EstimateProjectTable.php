...
namespace App\Http\Livewire\EstimateProjectV2\DataTable;
...
final class EstimateProjectTable extends PowerGridComponent
{
    ...
    */
    ...
    {
        return EstimatePrepareV2::query()
            ->select(
                'estimate_master_v2.id',
                'estimate_master_v2.estimate_id',
                'estimate_master_v2.total_amount',
                'estimate_statuses.status',
                DB::raw('ROW_NUMBER() OVER (ORDER BY estimate_master_v2.id) as serial_no')
            )
            ->join('estimate_user_assign_records', 'estimate_user_assign_records.estimate_id', '=', 'estimate_prepares_v2.estimate_id')
            ->join('estimate_master_v2', 'estimate_master_v2.estimate_id', '=', 'estimate_prepares_v2.estimate_id')
            ->join('estimate_statuses', 'estimate_statuses.id', '=', 'estimate_master_v2.status')
            ->where('estimate_user_assign_records.estimate_user_type', '=', 5)
            ->whereIn('estimate_master_v2.status', [1, 10, 12])
            ->where('estimate_prepares_v2.created_by', Auth::user()->id)
            ->groupBy('estimate_master_v2.estimate_id','estimate_master_v2.id','estimate_statuses.status');
    }
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ->addColumn('id')
            ->addColumn('estimate_master_v2.estimate_id')
            ->addColumn('SOR.estimate_master_desc')
            ->addColumn('estimate_master_v2.total_amount')
    ...
    */
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('DESCRIPTION', 'SOR.estimate_master_desc'),
