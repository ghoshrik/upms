	...
	public function scopeSorReportsDeptCategory($query)
	{
		return $query->join('sor_category_types', 'dynamic_table_header.dept_category_id', '=', 'sor_category_types.id')
			->select(
				...
				DB::raw("COUNT(CASE WHEN dynamic_table_header.is_approve = '-11' THEN 1 END) as pending_approval_count"),
				DB::raw("COUNT(CASE WHEN dynamic_table_header.is_verified = '-09' THEN 1 END) as verified_count")
			)
